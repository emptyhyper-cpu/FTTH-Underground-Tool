...
const P = {
  ...
  "Duct in Sand": 450, "Road Cut": 850, "Break PB": 850,
...
async function doImport(file, setters) {
  ...
  // Civil
  const newCivil = {};
  const cMap = { 79:"A8 Pole",80:'Flex RT 1/2"',81:'Flex RT 3/4"',82:'Flex RT 1"',83:'Flex 1/2"',84:'Flex 3/4"',85:'Flex 1"',86:'IMC 1/2"',87:'IMC 3/4"',88:'IMC 1"',89:'uPVC 1/2"',90:'uPVC 3/4"',91:'uPVC 1"',92:'PVC 1/2"',93:'PVC 3/4"',94:'PVC 1"',95:'HDPE 1/2"',96:'HDPE 3/4"',97:'HDPE 1"',98:'HDPE 2"',100:"Duct in Sand",101:"Road Cut",102:"Break PB",103:"PB 4x4 Galv",104:"PB 6x6 Galv",105:"PB 10x10 Galv",106:"PB 12x12 Galv",107:"PB 4x4 Plastic",108:"PB 6x6 Plastic",109:"PB 10x10 Plastic",110:"PB 12x12 Plastic",111:"HH-01 CONC" };
  Object.entries(cMap).forEach(([r, k]) => { if (q(r) > 0) newCivil[k] = String(q(r)); });
  setters.setCivilVals(newCivil);

  // Lastmile cable
  const newLm = {};
  const lMap = { 113:"Flat 1C",114:"Flat 2C",115:"Round 1C",116:"Round 2C",117:"Armoured 1C",118:"Armoured 2C",119:"Duct Flat 1C",120:"Duct Flat 2C" };
  Object.entries(lMap).forEach(([r, k]) => { if (q(r) > 0) newLm[k] = String(q(r)); });
  setters.setLmSels(newLm);

  // Lastmile per-home
  if (q(121) > 0) setters.setTbQty(String(q(121)));
  if (q(124) > 0) setters.setSpliceHouseQty(String(q(124)));
  if (q(125) > 0) setters.setSpliceDrop(String(q(125)));
  if (q(126) > 0) setters.setBreakSewer(String(q(126)));

  // Management
  if (q(128) > 0) setters.setSurvey(String(q(128)));
  if (q(129) > 0) setters.setIgis(String(q(129)));
...
async function doExport(state) {
  ...
  // Lastmile
  const lR = { "Flat 1C":113,"Flat 2C":114,"Round 1C":115,"Round 2C":116,"Armoured 1C":117,"Armoured 2C":118,"Duct Flat 1C":119,"Duct Flat 2C":120 };
  Object.entries(state.lmSels).forEach(([k, q]) => { if (lR[k]) set(lR[k], nv(q)); });
  set(121, state.tbQty !== "" ? nv(state.tbQty) : homesN);
  set(124, state.spliceHouseQty !== "" ? nv(state.spliceHouseQty) : homesN);
  set(125, nv(state.spliceDrop)); set(126, nv(state.breakSewer));
  // Management
  set(128, nv(state.survey)); set(129, nv(state.igis)); set(130, nv(state.testReport));
  set(131, state.accQty !== "" ? nv(state.accQty) : homesN);
  set(132, state.mgmtQty !== "" ? nv(state.mgmtQty) : homesN);
  set(250, homesN);
  ...
  const descMap = {
    ...
    100:"Install duct in Sand",
    101:"Open Cut Road Concrete and repair",
    102:"BREAK THROGH PB.",
    103:'Pull Box size 4"x4"x4" Galvanized',
    104:'Pull Box size 6"x6"x4" Galvanized',
    105:'Pull Box size 10"x10"x6" Galvanized',
    106:'Pull Box size 12"x12"x6" Galvanized',
    107:'Pull Box size 4"x4"x4" Plastic',
    108:'Pull Box size 6"x6"x4" Plastic',
    109:'Pull Box size 10"x10"x6" Plastic',
    110:'Pull Box size 12"x12"x6" Plastic',
    111:"HH-01 CONC",
    113:"SM-AD Dropwire Flat Type Install in Duct 1C",
    114:"SM-AD Dropwire Flat Type Install in Duct 2C",
    115:"SM-AD Dropwire Round Type Install in Duct 1C",
    116:"SM-AD Dropwire Round Type Install in Duct 2C",
    117:"SM-AD Dropwire Armoured Round Type Install in Duct 1C",
    118:"SM-AD Dropwire Armoured Round Type Install in Duct 2C",
    119:"SM-D Duct Dropwire Flat Type Install in Duct 1C",
    120:"SM-D Duct Dropwire Flat Type Install in Duct 2C",
    121:"Terminal Box Outlet 1C with Adapter SC/APC include SC/APC Pigtail",
    122:"Terminal Box Outlet 2C with Adapter SC/APC include SC/APC Pigtail",
    123:"Terminal Box Face Plate",
    124:"Fusion Splice Terminate @House with SC/APC Pigtail",
    125:"Splice core fiber drop wire",
    126:"break sewer to Post Box",
    128:"Survey & Drawings",
    129:"IGIS",
    130:"Test & Document Report",
    131:"Accessories",
    132:"Management",
    249:"ONU Installation",
  };
    ...
    <>
      ...
      <div style={{ background: `linear-gradient(90deg,${C.headerBg},${C.headerLight})`, color: C.white, padding: "11px 16px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 2px 10px rgba(46,125,50,0.25)" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 10, color: "#c8f7c5", fontFamily: FONT }}>AIS Fibre · Price List V7</div>
        </div>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "12px 12px 60px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <Section num="5" title="Civil Work — ท่อ / ถนน / Pull Box" total={sub.s5}>
          ...
          <QtyRow label="Install duct in Sand (ฝังท่อในทราย)" unit="M" priceKey="Duct in Sand" {...sc("Duct in Sand")} />
